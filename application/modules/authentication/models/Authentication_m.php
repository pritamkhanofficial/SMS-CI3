...
class Authentication_m extends CI_Model {
    ...
    public function authentication() {
        ...
        if($this->form_validation->run() == FALSE) {
            return array('type'=>'load_view', 'page'=>'login');
        }
        ...
        $rs_acc_chk = $this->db->get('user_login')->num_rows();
        ...
        if (($rs_acc_chk) == 0) {
            ...
            return array('type'=>'redirect', 'page'=>'auth');
        }
        ...
        elseif($row->is_block == 1) {
            ...
            $this->session->set_flashdata('type', 'warning');
            $this->session->set_flashdata('title', 'Deactivated!');
            $this->session->set_flashdata('msg', 'Your account is deactivate, please contact with administrator.');
            return array('type'=>'redirect', 'page'=>'auth');
        }
        ...
        $getUser = $this->getUserByRoleID($row->role_id, $row->user_id);
        
        $getConfig = $this->db->get_where('global_settings', array('global_settings_id' => 1))->row_array();
        // echo "<pre>"; print_r($getConfig); die();
        $email = isset($getUser->email) ? $getUser->email : 'N/A';
        $session_data = array(
            'user_id' => $getUser->user_id,
            'user_tbl_id' => $getUser->user_tbl_id,
            'full_name' => $getUser->full_name,
            'username' => $getUser->username,
            'email' => $email,
            'phone' => $getUser->phone,
            'image' => $getUser->profile_img,
            'role_id' => $getUser->role_id,
            'role_name' => $getUser->display_name,
            'session_id' => $getConfig['session_id'],
            'loggedin' => true,
        );
        $this->session->set_userdata($session_data); //creating session
        // echo $this->session->full_name; die();
        //$this->session->set_flashdata('type', 'success');
        //$this->session->set_flashdata('msg', 'Welcome to dashboard area');
        return array('type'=>'redirect', 'page'=>'dashboard'); //redirect to admin dashboard
    ...
    public function logout() { // logout
        ...
        $this->session->set_flashdata('title', 'Yes!');
        $this->session->set_flashdata('type', 'success');
        $this->session->set_flashdata('msg','You have been logged-out successfully.');
        return redirect(base_url('auth')); //redirect to user login page
    } // /.logout()
    ...
    private function getUserByRoleID($role_id, $user_id)
    {
        /* Role ID :  1=admin, 2=teacher, 3=accountant, 4=student, 5=parent  */
        if ($role_id == 1) {
            $this->db->join('roles as role_data', 'role_data.id = user_login.role_id', 'left');
