...
function get_session_list()
{
    $CI = &get_instance();
    $session_array = array();
    $academic_year = $CI->db->get('schoolyear')->result();
    foreach ($academic_year as $key => $value) {
        $session_array[$value->session_id] = $value->session;
    }
...
function get_global_setting($name = '')
{
    $CI = &get_instance();
    $name = trim($name);
    $CI->db->where('global_settings_id', 1);
    $CI->db->select($name);
...
{
    $ci = &get_instance();
    return $ci->session->userdata('user_tbl_id');
}
...
function loggedin_role_name()
{
    ...
    return $CI->db->select('display_name')->where('id', $roleID)->get('roles')->row()->display_name;
}
...
function get_type_name_by_id($table, $wherecol ='', $type_id = '', $field = '')
{
    $CI = &get_instance();
    ...
    $get = $CI->db->select($field)->from($table)->where($wherecol, $type_id)->limit(1)->get()->row_array();
    return $get[$field];
}
